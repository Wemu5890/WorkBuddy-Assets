...
def import_pet_from_folder(source_folder, pet_name, assets_base_dir):
    """
    支持全新的命名规范，如 "Failed_失败的_8帧.png"
    也向下兼容旧的 "待机_6.png"
    """
    target_pet_dir = os.path.join(assets_base_dir, pet_name)
    os.makedirs(target_pet_dir, exist_ok=True)
    count = 0
    ...
    for filename in os.listdir(source_folder):
        ...
        # 开始移动文件并生成配置
        action_dir = os.path.join(target_pet_dir, folder_name)
        os.makedirs(action_dir, exist_ok=True)
        
        src_path = os.path.join(source_folder, filename)
        dest_path = os.path.join(action_dir, 'sprite.png')
        ...
        with open(os.path.join(action_dir, 'config.json'), 'w', encoding='utf-8') as f:
